...
type BlobStorageBackend interface {
	...
	Upload(ctx context.Context, uri string, data []byte, metadata UploadMetadata) (err)
}

// BackendRegistry maps URIs (typically based on prefix) to an associated backend.
type BackendRegistry interface {
	// GetBackendForUri returns the backend for the specified URI if available.
	GetBackendForUri(uri string) (BlobStorageBackend, err)
}
